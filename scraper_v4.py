"""
競馬予想AI - scraper_v4.py（enhanced_scorer_v6対応版）
最終更新: 2026年2月16日
...
try:
    from enhanced_scorer_v6 import RaceScorer
except ImportError as e:
    logger.error(f"Import error: {e}")
    raise ImportError("enhanced_scorer_v6.py が必要です")
        ...
        try:
            ...
            for idx, row in enumerate(rows):
                ...
                try:
                    ...
                    # 1着の場合、2着との着差（winner_margin）を all_horses_results から取得
                    winner_margin = 0.0
                    if chakujun == 1:
                        ahr = race_stats.get('all_horses_results', [])
                        second_list = [h for h in ahr if h.get('chakujun') == 2]
                        if second_list:
                            winner_margin = round(abs(second_list[0].get('goal_time_diff', 0.0)), 3)
                    ...
                    history.append({
                        ...
                        'winner_margin': winner_margin,    # 1着時の2着との差（秒）
                        'weight': weight,
                        'last_3f': last_3f,
