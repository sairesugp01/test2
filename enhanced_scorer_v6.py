...
class RaceScorer:
    ...
    def _calculate_dominant_win_bonus(self, history_data: List[Dict]) -> float:
        """
        圧勝ボーナス（直近3走の勝利時・着差に応じて加点）

        1着で2着に大差をつけた実績を評価する。
        単純な「勝ち」より「どれだけ強く勝ったか」を重視。

        ボーナス基準（2着との着差）:
          0.1s以上  → +1.0点（クビ差・接戦勝ち）
          0.3s以上  → +2.0点（1馬身程度）
          0.6s以上  → +4.0点（3馬身程度）
          1.0s以上  → +6.0点（5馬身程度）
          1.5s以上  → +9.0点（大差・圧勝）
          2.0s以上  → +12.0点（スナッピードレッサ級の圧勝）

        直近走ほど高ウェイト（1走前1.0倍、2走前0.7倍、3走前0.5倍）
        最大加点: +12.0点（1走前での圧勝）
        """
        if not history_data:
            return 0.0
        ...
        for i, race in enumerate(history_data[:3]):
            if race.get('chakujun') != 1:
                continue
            ...
            if self.debug_mode:
                logger.debug(
                    f"  圧勝ボーナス: {i+1}走前 1着・2着との差{margin:.2f}s"
                    f" → {pts:.1f}点 × 減衰{decay:.1f} = {pts*decay:.1f}点"
                )
    ...
    def calculate_total_score(self, current_weight: float, target_course: str, target_distance: int,
        ...
        # 10+. 圧勝ボーナス
        dominant_win_bonus = self._calculate_dominant_win_bonus(history_data)
        ...
        return {
            ...
            'dominant_win_bonus': dominant_win_bonus,
    ...
    def format_score_breakdown_verbose(self, result: Dict, target_distance: int,
        ...
        clp = result.get('consecutive_loss_penalty', 0)
        # ─── 圧勝ボーナス ────────────────────────────────────────
        dwb = result.get('dominant_win_bonus', 0.0)
        lines.append(f"\n▼ 圧勝ボーナス: +{dwb:.1f}点")
        DECAY = [1.0, 0.7, 0.5]
        win_found = False
        for i, race in enumerate(history_data[:3]):
            ...
            win_found = True
            if not margin or margin <= 0:
                lines.append(f"  {i+1}走前 1着: 2着との着差データなし（キャッシュ更新で取得可）")
                continue
            if margin >= 2.0:   pts = 12.0
            elif margin >= 1.5: pts = 9.0
            elif margin >= 1.0: pts = 6.0
            elif margin >= 0.6: pts = 4.0
            elif margin >= 0.3: pts = 2.0
            elif margin >= 0.1: pts = 1.0
            else:               pts = 0.0
            decay = DECAY[i]
            rname = race.get('race_name', '')
            lines.append(
                f"  {i+1}走前 {rname} 1着・2着との差{margin:.2f}s"
                f" → {pts:.1f}点 × 減衰{decay:.1f} = {pts*decay:.1f}点"
            )
        if not win_found:
            lines.append("  （直近3走に1着なし）")
