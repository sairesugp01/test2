...
try:
    ...
    st.error("""
    **解決方法**:
    1. `scraper_v3_fixed.py` と `enhanced_scorer_fixed_v4.py` が同じディレクトリにあることを確認
...
SCHEDULE = {
    "20260207": {"東京": (1, 3), "京都": (2, 3), "小倉": (1, 5)},
    "20260208": {"東京": (1, 4), "京都": (2, 4), "小倉": (1, 6)}
}
...
if mode == "個別レース":
    ...
    if analyze_clicked:
        with st.spinner(f"{race_no}R 分析中..."):
            st.session_state['race_info'] = NetkeibaRaceScraper().get_race_data(rid)
